...
void main() {
  test(
    ...
    () {
      ...
      var results = container.read(historySearchItemsProvider);
      ...
      results = container.read(historySearchItemsProvider);
      expect(results.map((item) => item.title).toList(), [
        '劳动仲裁风险分析',
        '劳动仲裁申请书草稿',
      ]);
  ...
  test('historySearchItemsProvider restores full list after reset', () {
    final container = ProviderContainer();
    addTearDown(container.dispose);
    ...
    final results = container.read(historySearchItemsProvider);
